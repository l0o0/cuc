...
class TAB2(QtWidgets.QWidget):
    ...
    def restoreButtonAction(self):
        ...
        if button:
            ...
            taskline = self.doneTask.tasklines[row]
            taskline.mask = None
            taskline.completion_date = None
            self.restore_trigger.emit(taskline)
            del self.doneTask.tasklines[row]
    ...
    def createTaskRow(self, donetask, row, tablewidget, config=None):
        ...
        style_temp = "QLabel{padding-top:0;padding-left:5px;font-family:Arial,SimHei,NotoColorEmoji;font-size:%spx}"
    ...
    def saveDoneTask(self, done):
        done_texts = [t.format_text() + '\n' for t in self.doneTask.tasklines if getattr(t, 'status', None)  != 'saved']
        with open(done, 'a', encoding='utf-8') as handle:
            handle.writelines(done_texts)
            print('save done.txt, %s' % len(self.doneTask.tasklines))